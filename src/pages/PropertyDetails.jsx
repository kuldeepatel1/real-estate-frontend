...
export default function PropertyDetails() {
  ...
  const { favoritesByPropertyId, list: favorites } = useSelector((state) => state.favorites);
  ...
  // Use O(1) lookup map for favorite status check
 ...
 useEffect(() => {
  ...
  const handleFavorite = () => {
    ...
    // Dispatch the action - Redux will handle the state update
    if (isFavorite) {
  ...
  <div className="relative h-96">
   ...
   {/* ⬅ Previous - Hide when at first image */}
{currentImageIndex > 0 && (
<button
  onClick={() =>
